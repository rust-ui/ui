...
pub fn highlight_toml_manually(code: &str) -> String {
    ...
    for line in code.lines() {
        ...
        if trimmed.starts_with('[') && trimmed.ends_with(']') {
            let _ = writeln!(html_output, "<span class=\"syntax__keyword\">{}</span>", html_escape::encode_text(line));
        } else if trimmed.starts_with('#') {
            let _ = writeln!(html_output, "<span class=\"syntax__comment\">{}</span>", html_escape::encode_text(line));
