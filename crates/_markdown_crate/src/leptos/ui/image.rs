...
pub fn image_md(props: MdComponentProps) -> impl IntoView {
    ...
    let class = props.attributes.get("class").and_then(|v| v.clone()).unwrap_or_else(|| "max-w-full".to_string());

    view! { <img src=src alt=alt width=width class=class loading="lazy" decoding="async" /> }
}
